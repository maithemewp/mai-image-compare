...
- Horizontal and vertical, optional labels, per-image alt text overrides in a collapsed panel.
- Site-wide defaults for start position, slide on hover and drag anywhere, set in code through `mai_image_compare_defaults`. Drag anywhere greys out while slide on hover is on, since hover leaves nothing to restrict. A block can deviate; one that does not follows the filter on the next page view.
